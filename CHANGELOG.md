...
- **TABLE entity (#22):** real tables for bills of materials, revision blocks and hole schedules — rows and columns with stored sizes, merged cells, per-cell alignment, and borders/grid/text derived from a named **table style**. Selectable and movable as one object, with grips to resize columns. Cell editing and DXF `ACAD_TABLE` are staged and documented.
